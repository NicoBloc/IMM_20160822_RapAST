...
masterplot <- function(d) {
  ...
  if (nlevels(d$phenotypes) > 2) {
    ptVec <- c("all non-wt combined", ptVec)
    dTmpDupl <- rbind(dTmpDupl, dTmpDupl %>% mutate(phenotypes="all non-wt combined"))
  }
...
lineplotHombachMaster <- function(d) {
  ...
  pltNonWt[[nlevels(dTmp$phenotypes)]] <- lineplotHombachPanel(dTmp[dTmp$phenotypes %in% allNonWt, ])
  pltWtAndNonWt[[nlevels(dTmp$phenotypes)]] <- lineplotHombachPanel(dTmp)
...
polishPlot <- function(plt) {
  plt + xlab('time/h') + ylab('diameter/mm') + coord_cartesian(ylim=c(6, 40)) + theme(legend.position="top")
}
...
dRaw <- read.csv("../../Data/Daten_für Nico_rapid_AST_Paper_V5.csv", header=TRUE, sep=';', na.strings='') %>% gather('h', 'diameter', 4:7)
...
```{r, echo=FALSE, warning=FALSE, fig.height=6.5, results='asis'}
showAllPlots <- FALSE
...
for (ab in levels(dRaw$antibiotic)) {
  ...
  for (sp in levels(d$organism)) {
    ...
    cat('\n\n\n\n')
    dTmp %>% group_by(phenotypes) %>% summarise(n=n_distinct(checkin)) %>% kable %>% print
    cat('\n\n\n\n')
    ...
    if (showAllPlots) {
      ...
      bxplt[[nlevels(dTmp$phenotypes)]] <- myBoxplot(dTmp %>% mutate(phenotypes=isWt)) + ggtitle('all non-wt combined')
      cat('\n\n\\pagebreak\n')
...
```{r AUC, echo=FALSE, warning=FALSE, fig.height=8, results='asis'}
