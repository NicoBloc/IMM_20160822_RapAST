...
dRaw <- read.csv("../../Data/Daten_2._rapid_AST_Paper_V4_2.csv", header=TRUE, sep=';', na.strings='') %>% gather('h', 'diameter', 4:7)
...
lineplotWFacets <- function(d) {
  return(polishPlot(ggplot(dTmp, aes(x=h, y=diameter, group=checkin, colour=isWt)) + geom_line() + geom_point() + facet_wrap(~ phenotypes) + scale_x_continuous(breaks=c(6, 8, 12, 18), minor=NULL)))
}
...
for (ab in levels(dRaw$antibiotic)) {
  ...
  for (sp in levels(d$organism)) {
    ...
    # pltWt <- lineplotHombach(dTmp[dTmp$phenotypes == wt, ])
    # 
    # pltNonWt <- list()
    # pltWtAndNonWt <- list()
    # for (i in 1:(nlevels(dTmp$phenotypes) - 1)) {
    #   nonWt <- allNonWt[i]
    #   pltNonWt[[i]] <- lineplotHombach(dTmp[dTmp$phenotypes == nonWt, ])
    #   pltWtAndNonWt[[i]] <- lineplotHombach(dTmp[dTmp$phenotypes %in% c(wt, nonWt), ])
    # }
    # 
    # pltNonWt[[nlevels(dTmp$phenotypes)]] <- lineplotHombach(dTmp[dTmp$phenotypes %in% allNonWt, ])
    # pltWtAndNonWt[[nlevels(dTmp$phenotypes)]] <- lineplotHombach(dTmp)
    # 
    # allPlots <- list()
